	 ...
	 if(FileOpenDialog1->Execute()){
		...
		if(FileExists(FileOpenDialog1->FileName)){
			try {
				...
				streamAnkete->WriteBuffer(&header, sizeof(Header));

				for(Anketa i : anketeGlobal) {
					streamAnkete->WriteBuffer(&i, sizeof(Anketa));
				 }
...
void __fastcall TAnketeAdminForm::Button3Click(TObject *Sender)
{
	 if(FileOpenDialog1->Execute()){
		  if(FileExists(FileOpenDialog1->FileName)){
			...
			TFileStream *streamAnkete = new TFileStream(FileOpenDialog1->FileName, fmOpenRead);

			try {
				Header header;
				streamAnkete->ReadBuffer(&header, sizeof(Header));
				...
				for(int i = 0; i < brojAnketa; i++){
						 streamAnkete->ReadBuffer(&anketa, sizeof(Anketa));
