...

{
     ...
     SettingsForm->Hide();
	 NaloziForm->Hide();
	 PodrskaForm->Parent = Panel2;
	 PodrskaForm->Align  = alClient;
	 PodrskaForm->Show();
}
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
	 ...
	 if(Reg->OpenKey(key, true)) {
		 Korisnik::GetInstance().setUserName(Reg->ReadString("user"));
		 Korisnik::GetInstance().setLoginTime(Reg->ReadDateTime("lastLogin"));
		 Korisnik::GetInstance().setIsAdmin(Reg->ReadBool("admin"));
		 Label1->Caption = "Dobrodošao, " +  Korisnik::GetInstance().getUserName() + "!";
     }

	 if(Korisnik::GetInstance().getIsAdmin()) {
